...
import { 
    ...
    MdHowToVote, MdLock, MdHistoryEdu, MdVisibility ,MdSentimentSatisfiedAlt
} from "react-icons/md";
    ...
    return (
        <div className="detail-container">
            ...
            <div style={{ 
                ...
                >
                    {isMyBookmark ? (
                        <MdStar style={{ fontSize: '1.8rem', color: '#ffca28' }} />
                    ) : (
                        <MdStarBorder style={{ fontSize: '1.8rem', color: '#ccc' }} />
                    )}
                ...
                {detail.type === 'survey' ? (
                    <div className="survey-vote-container" style={{ background: '#f9f9f9', padding: '30px', borderRadius: '15px', border: '1px dashed #ddd' }}>
                        <h3 style={{ fontSize: '18px', marginBottom: '20px', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                            {loginUser ? (
                                <><MdHowToVote style={{ color: '#8a2130' }} /> 원하시는 항목을 클릭하여 투표하세요!</>
                            ) : (
            ...
            <div className="detail-footer" style={{ marginTop: '50px', textAlign: 'center' }}>
                <button 
                    className="btn-back" 
                    onClick={onBack} 
                    style={{ padding: '10px 20px', backgroundColor: '#6c757d', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}
                >
                ...
                {loginUser && detail.userId === loginUser && (
                    <div style={{ display: 'inline-block', marginLeft: '10px' }}>
                        <button 
                            className="btn-edit" 
                            onClick={onEdit} 
                            style={{ padding: '10px 20px', backgroundColor: '#8a2130', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer', marginRight: '10px', display: 'inline-flex', alignItems: 'center' }}
                        >
                        ...
                        <button 
                            ...
                            style={{ padding: '10px 20px', backgroundColor: '#093c71', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}
                        >
            ...
            <div className="related-section" style={{ marginTop: '40px' }}>
                <h4 style={{ fontSize: '16px', marginBottom: '20px', color: '#8a2130', display: 'flex', alignItems: 'center' }}>
                    <MdPushPin style={{ marginRight: '5px', color: '#8a2130', transform: 'rotate(45deg)' }} /> 
                    게시글 더보기
                </h4>
                
                {relatedPosts.next.length > 0 && (
                    <div style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                        {relatedPosts.next.map(post => renderCard(post, '다음'))}
                    </div>
                )}
                
                {relatedPosts.prev.length > 0 && (
                    <div style={{ display: 'flex', gap: '15px' }}>
                        {relatedPosts.prev.map(post => renderCard(post, '이전'))}
                    </div>
                )}
            </div>
        </div>
